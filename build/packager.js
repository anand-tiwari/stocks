...
module.exports = {
  doPackage: function (mode) {
    ...
    // outputResource - stock-ui-resources-1.0.0-1.zip its something like this.
    // after extract will structure like this - stock-ui-resources/1.0.0-1
    this.archive(output, function (archive) {
      ...
      // add 4xx nad 5xx static pages in target directory
      const customPages = ['403.html', '404.html']
      customPages.forEach((customPage, index) => {
        const targetPath = path.join(projectDir, target, customPage)
        fse.copyFile('./build/custom-pages/' + customPage, targetPath, (err) => {
      ...
      // this step does not make sense. it pushes resources in artifactory
      // although it is not used in dockerization. and we don't use static repo
      // everything is embeded in Docker image
